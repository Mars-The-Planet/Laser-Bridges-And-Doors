...
public class Laserbridges {
    ...
    public static class ClientModEvents {
        ...
        public static void registerBlockColorHandlers(RegisterColorHandlersEvent.Block event) {
            event.register((state, level, pos, tintIndex) -> ((((int)(DyeColor.byId(state.getValue(COLOR)).getTextureDiffuseColors()[0] * 255)) << 16) | (((int)(DyeColor.byId(state.getValue(COLOR)).getTextureDiffuseColors()[1] * 255)) << 8) | (int) ((DyeColor.byId(state.getValue(COLOR)).getTextureDiffuseColors()[2] * 255))), BRIDGE_SOURCE_BLOCK.value(), LASER_FENCE_SOURCE_BLOCK.value(), LASER_BLOCK.value(), LASER_FENCE_BLOCK.value());
        }
    }
